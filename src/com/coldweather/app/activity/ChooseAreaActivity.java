...
public class ChooseAreaActivity extends Activity {
	...
	private void queryCounties() {
		countyList = coldWeatherDB.loadCounties(selectedcCity.getId());
		...
		HttpUtil.sendHttpRequest(address, new HttpCallbackListener() {
			...
			public void onFinish(String response) {
				...
				if ("province".equals(type)) {
					result = Utility.handleProvincesResponse(coldWeatherDB, response);
				} else if ("city".equals(type)) {
					result = Utility.handleCitiesResponse(coldWeatherDB, response, selectedProvince.getID());
				} else if ("county".equals(type)) {
					result = Utility.handleCountiesResponse(coldWeatherDB, response, selectedcCity.getId());
				}
				if (result) {
